...
function isPalindrome(str) {
  ...
  return cleanedStr === cleanedStr.split('').reverse().join('').toLowerCase();
}
...
function findLongestWord(sentence) {
  const cleanedStr = sentence.replace(/[^a-z0-9]/gi, ' ');
  const str = cleanedStr.split(' ');
  let max = 0;
  let maxIndex = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].length > max) {
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((acc) => acc.split('').reverse().join(''))
    .join(' ');
}
...
function invertCase(str) {
  return str
    .split('')
    .map((acc) =>
      acc === acc.toLowerCase() ? acc.toUpperCase() : acc.toLowerCase()
    )
    .join('');
}
...
function extractNameFromTemplate(value) {
  return value
    .slice(value.indexOf(',') + 1, value.indexOf(value.length - 1))
    .trim();
}
...
function encodeToRot13(str) {
  ...
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    if (input.includes(s[i])) {
      res += output[input.indexOf(s[i])];
    } else {
      res += s[i];
    }
  }
